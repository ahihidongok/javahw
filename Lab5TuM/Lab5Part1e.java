...
import static lab5part1e.Lab5Part1e.printTitles;
import static lab5part1e.Lab5Part1e.sortArray0;
...
public class Lab5Part1e {
   ...
   Scanner inData = null;
       ...
       // printarray(names,years,seriesNum);
        ...
        System.out.println("\nBefore remove titles");
        sortArray(titles);
        printTitles(titles,titles.length);
    
        removeTitle(titles,titles.length);
    }
...
for ( int i=0;i<80;i++)
{
    ...
    public static void sortArray2 (String[]titles, int counter)
    {   
        String temp1 = "";
        
        for (int j = 0; j < counter; j++ )
        {
            if (titles[j].trim().startsWith("The "))
            {
                temp1 = titles[j].trim().substring(4);
                titles[j] = temp1+", The";
               
            }
            else if (titles[j].trim().startsWith("A "))
            {    
                temp1 = titles[j].trim().substring(2);
                titles[j] = temp1+", A";
            }
            else
            {
        ...
        sortArray(titles);
        System.out.println("\n\n This is titles after sort\n\n");
        printTitles(titles,counter);
...
public static int removeTitle(String[] Titles, int count)
    {
        System.out.println("Please enter the title that you want to remove");
        Scanner sc = new Scanner(System.in);
        String addedTitle = sc.nextLine();
        sortArray0(Titles);
        System.out.println("\n\nAfter remove title");
        ...
        for (int i=0; i<15; i++)
        {
            if (Titles[i].trim().compareTo(addedTitle.trim()) == 0 ) 
            { 
                if (found == false)
                {   
                    index = i;
        ...
        {   count--;
            //System.out.println(index);
  //move row 1 down for new entry
            for (int l=index;l<Titles.length-1;l++)    
            {
              Titles[l] = Titles[l+1];  
            }
        }
        ...
        {
            System.out.println("The input title does not match any entries");
            return count;
        }
        found = false;
        for (int k=Titles.length-1;k>0;k--)
        {
           if ((Titles[k].trim()!="~") && (found == false))
           {
              Titles[k]="~";
              found=true;
