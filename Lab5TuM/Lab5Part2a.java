...
public class Lab5Part2a {
    ...
    public static void main(String[] args) {
       ...
       addTitleAL(titlesAL,titlesAL.size());
...
for ( int i=0;i<80;i++)
{
    ...
    public static int addTitleAL (ArrayList TitleAL, int count)
    {
        System.out.println("Please input the title you want to add");
        Scanner sc = new Scanner(System.in);
        String inputtitle = sc.nextLine();
        for (int h=0;h<TitleAL.size() ;h++)
        {
            if (TitleAL.get(h).equals(inputtitle.trim()) )
            {
        ...
        TitleAL.add(inputtitle);
        Collections.sort(TitleAL);
        System.out.println("\nAfter add\n");
        for (int i=0; i<TitleAL.size(); i++) 
            System.out.print(TitleAL.get(i)+"\n"); 
